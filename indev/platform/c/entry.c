...
void freestack(Stack* stack) { /* XXX this does nothing now */ }
...
unsigned long long ground(unsigned long long w, unsigned long long s,
...
unsigned long long grun(void) {
  ...
  unsigned long long workloads[wsize][3] = {
      {13, 29, 17}, {21, 37, 11}, {31, 23, 41}, {43, 19, 47},
      {13, 29, 17}, {21, 37, 11}, {31, 23, 41}, {43, 19, 47},
  };
  ...
  r = ground(work[0], times[0], workloads[0][0], workloads[0][1],
             workloads[0][2]);
  r = mix64(r);
  shuffle(order, &r);
...
unsigned long long grandom(void) {
  ...
  for (i = 0; i < c; i++) seed = mix64(seed ^ grun());
