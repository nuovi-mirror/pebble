...
static ValueTypes numtype(Value l, Value r) {
  if (l.Type == type_flt || r.Type == type_flt) return type_flt;

  if (l.Type == type_sword || r.Type == type_sword) return type_sword;
...
static unsigned long numword(Value v) { return v.as.word; }
...
static double numflt(Value v) {
  switch (v.Type) {
    case type_flt:
      return v.as.flt;
    case type_sword:
      return (double)v.as.sword;
    default:
      return (double)v.as.word;
  }
}
...
static Value h_NumericAdd(Value l, Value r) {
  ...
  switch (numtype(l, r)) {
    case type_flt:
      v.Type = type_flt;
      v.as.flt = numflt(l) + numflt(r);
      break;
    case type_sword:
      v.Type = type_sword;
      v.as.sword = numsword(l) + numsword(r);
      break;
    default:
      v.Type = type_word;
      v.as.word = numword(l) + numword(r);
      break;
  }
...
static Value h_NumericDiv(Value l, Value r) {
  ...
  switch (numtype(l, r)) {
    case type_flt:
      v.Type = type_flt;
      v.as.flt = numflt(r) == 0.0 ? 0.0 : numflt(l) / numflt(r);
      break;
    case type_sword:
      v.Type = type_sword;
      v.as.sword = numsword(r) == 0 ? 0 : numsword(l) / numsword(r);
      break;
    default:
      v.Type = type_word;
      v.as.word = numword(r) == 0 ? 0 : numword(l) / numword(r);
      break;
  }
  ...
  switch (node->Op) {
    ...
    case ExprOp_StringEndsWith: {
      ...
      for (unsigned long i = start; i <= end; i++)
        if (cmpstrn(ls + i, rs, rl) == 0) return mkbool(1);
...
Value evalexprdata(ExprNodeData data, VarMap* vars, Arena* persistAlloc) {
  if (data.Type == ExprDataVal) return resolveleaf(data.value, vars);
