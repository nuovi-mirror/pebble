...
void remove_target(void) {
  if (!dryrun && !print && !precious && target &&
      !(target->n_flag & (N_PRECIOUS | N_PHONY)) &&
 ...
 */
static void touch(struct name* np) {
  if (dryrun || !silent) printf("touch %s\n", np->n_name);
 ...
 */
static int docmds(struct name* np, struct cmd* cp) {
  int estat = 0;
  char *q, *command;
  ...
  for (; cp; cp = cp->c_next) {
    ...
    if (sdomake && *q != '\0') {  // Ignore empty commands
      ...
      target = np;
      status = system(cmd);
      if (!signore IF_FEATURE_MAKE_EXTENSIONS(&&posix)) free(cmd);
      ...
      if (status == -1 && !doinclude) {
        ...
        if (!posix && WIFSIGNALED(status)) remove_target();
#endif
        if (doinclude) {
          ...
          if (!quest || err_value == 127) {
            if (err_type)
              diagnostic("failed to build '%s' %s %d", np->n_name, err_type,
                         err_value);
            else
 ...
 */
static char* remove_suffix(const char* name, const char* tsuff) {
  char* base = NULL;
...
#endif
static int make1(struct name* np, struct cmd* cp, char* oodate, char* allsrc,
 ...
 */
static int timespec_le(const struct timespec* t, const struct timespec* p) {
...
int make(struct name* np, int level) {
  ...
  if (np->n_flag & N_DOING) error("circular dependency for %s", np->n_name);
  np->n_flag |= N_DOING;

  if (!np->n_tim.tv_sec) modtime(np);  // Get modtime of this file
  ...
  if (!(np->n_flag & N_DOUBLE)) {
    ...
    for (rp = np->n_rule; rp; rp = rp->r_next) {
      if (!rp->r_cmd) {
        ...
        if (!posix && (np->n_flag & N_PHONY)) continue;
#endif
        impdep = dyndep(np, &infrule, &tsuff);
        if (!impdep) {
          if (doinclude) return 1;
    ...
    if ((np->n_flag & N_DOUBLE)) {
      if (((np->n_flag & N_PHONY) || timespec_le(&np->n_tim, &dtim))) {
        if (!(estat & MAKE_FAILURE)) {
          estat |= make1(np, rp->r_cmd, oodate, allsrc, dedup, locdep, tsuff);
...
#if ENABLE_FEATURE_MAKE_EXTENSIONS
  if ((np->n_flag & N_DOUBLE) && impdep) free(infrule.r_dep);
#endif
  ...
  if (estat & MAKE_DIDSOMETHING) {
    modtime(np);
    if (!np->n_tim.tv_sec) clock_gettime(CLOCK_REALTIME, &np->n_tim);
