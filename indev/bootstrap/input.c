...
#define modify_words(v, m, lf, lr, fp, rp, fs, rs) modify_words(v, m, lf, lr, fs, rs)
#endif
/*
 ...
 */
static char *modify_words (const char *val, int modifier, size_t lenf, size_t lenr,
	const char *find_pref, const char *repl_pref, const char *find_suff,
...
#if ENABLE_FEATURE_MAKE_EXTENSIONS
enum { INITIAL = 0, SKIP_LINE = 1 << 0, EXPECT_ELSE = 1 << 1, GOT_MATCH = 1 << 2 };
