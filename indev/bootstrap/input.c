...
static char* gettok(char** ptr) {
  ...
  while (**ptr != '\0' && !isblank(**ptr)) (*ptr)++;  // Find end of word
 ...
 */
static char* skip_macro(const char* s) {
  while (*s && s[0] == '$') {
    if (s[1] == '(' || s[1] == '{') {
      char end = *++s == '(' ? ')' : '}';
      while (*s && *s != end) s = skip_macro(s + 1);
      if (*s == end) ++s;
    } else if (s[1] != '\0') {
      s += 2;
    } else {
 ...
 */
static char* modify_words(const char* val, int modifier, size_t lenf,
                          size_t lenr, const char* find_pref,
      ...
      if (find_pref != NULL && lenw + 1 >= lenf) {
        ...
        if (strncmp(word, find_pref, find_pref_len) == 0 &&
          ...
          if (!repl_suff) {
            ...
            word = newword =
                xconcat3(repl_pref, word + find_pref_len, repl_suff);
          }
        }
      } else
#endif
 ...
 */
static char* find_char(const char* str, int c) {
  const char* s;
 ...
 */
static char* find_colon(char* p) {
  char* q;

  for (q = p; (q = strchr(q, ':')); ++q) {
    if (posix && !(pragma & P_WINDOWS)) break;
    if (q == p || !isalpha(q[-1]) || q[1] != '/') break;
  }
  return q;
}
#else
...
char* expand_macros(const char* str, int except_dollar) {
  ...
  for (t = exp; *t; t++) {
    if (*t == '$') {
      ...
      if (*t == '{' || *t == '(') {
        t = find_char(t, *t == '{' ? '}' : ')');
        if (t == NULL) error("unterminated variable '%s'", s);
...
#endif
          {
            if (IF_FEATURE_MAKE_EXTENSIONS(
                    posix && !(pragma & P_EMPTY_SUFFIX) &&) lenf == 0)
              error("empty suffix%s", !ENABLE_FEATURE_MAKE_EXTENSIONS
                                          ? ""
      ...
      if ((mp = getmp(name))) {
        // Recursive expansion
        if (mp->m_flag) error("recursive macro %s", name);
#if ENABLE_FEATURE_MAKE_POSIX_2024
        // Note if we've expanded $(MAKE)
        if (strcmp(name, "MAKE") == 0) opts |= OPT_make;
#endif
        mp->m_flag = TRUE;
 ...
 * Return -1 on error.
 */
static int compare_strings(char* arg1) {
 ...
 * should be skipped.
 */
static int skip_line(const char* str1) {
  char *copy, *q, *token;
  bool new_level = TRUE;
  ...
  if ((token = gettok(&q)) != NULL) {
    if (strcmp(token, "endif") == 0) {
      if (gettok(&q) != NULL) error_unexpected("text");
      if (clevel == 0) error_unexpected(token);
      --clevel;
      ret = TRUE;
      goto end;
    } else if (strcmp(token, "else") == 0) {
      if (!(cstate[clevel] & EXPECT_ELSE)) error_unexpected(token);
    ...
    if (strcmp(token, "ifdef") == 0 || strcmp(token, "ifndef") == 0 ||
      ...
      if (match >= 0) {
        if (new_level) {
          // Start a new level.
          if (clevel == IF_MAX) error("nesting too deep");
          ++clevel;
          ...
          // at the new level.
          if ((cstate[clevel - 1] & SKIP_LINE)) cstate[clevel] |= GOT_MATCH;
        }
 ...
 */
static char* make_fgets(char* s, int size, FILE* fd) {
 ...
 */
static char* readline(FILE* fd, int want_command) {
  char *p, *str = NULL;
  int pos = 0;
  int len = 0;
  ...
  for (;;) {
    ...
    if (len - pos > 1 && make_fgets(str + pos, len - pos, fd) == NULL) {
      if (pos) return str;
      free(str);
      return NULL;  // EOF
    }
 ...
 * known suffixes.
 */
int is_inference_target(const char* s) {
  struct name* np;
  struct rule *rp1, *rp2;
 ...
 */
static int target_type(char* s) {
  int ret;
  static const char* s_name[] = {
      ".DEFAULT",  ".POSIX",       ".IGNORE",
      ".PRECIOUS", ".SILENT",      ".SUFFIXES",
#if ENABLE_FEATURE_MAKE_POSIX_2024
...
#endif
  {
    ...
    if (*s == '.' && is_suffix(sfx)) {
      if (s == sfx) {  // Single suffix rule
        ...
        *sfx = '\0';
        if (is_suffix(s)) ret = T_INFERENCE | T_NOPREREQ | T_COMMAND;
        *sfx = '.';
      }
    }
  }
...
#if ENABLE_FEATURE_MAKE_EXTENSIONS
static int ends_with_bracket(const char* s) {
 ...
 */
static char* process_command(char* s) {
  char *t, *u;
#if ENABLE_FEATURE_MAKE_POSIX_2024
  int len;
  char* outside;
...
static char* run_command(const char* cmd) {
  ...
  if ((fd = popen(cmd, "r")) == NULL) return val;
 ...
 * glob_t structure.
 */
static int wildcard(char* p, glob_t* gd) {
  int ret;
  char* s;
...
void pragmas_from_env(void) {
  ...
  q = var = xstrdup(env);
  while ((p = gettok(&q)) != NULL) set_pragma(p);
  free(var);
}
#endif
 ...
 */
static char* inline_command(char* line) {
...
void input(FILE* fd, int ilevel) {
  ...
  while (str1) {
    ...
    if (strncmp(p, "include", 7) == 0 && isblank(p[7])) {
      ...
      if (ilevel > 16) error("too many includes");
      ...
      while ((p = gettok(&q)) != NULL) {
        ...
        if ((ifd = fopen(p, "r")) == NULL) {
          if (!minus) error("can't open include file '%s'", p);
        } else {
          makefile = p;
          input(ifd, ilevel + 1);
          fclose(ifd);
    ...
    if (POSIX_2017 && *str == '\t') error("command not allowed here");
#endif
    if (find_char(str, '=') != NULL) {
      ...
      *q++ = '\0';  // Separate name and value
      ...
      if ((a = gettok(&p)) == NULL) error("invalid macro assignment");
      ...
      if (gettok(&p)) error("invalid macro assignment");
  ...
  try_target:
    ...
    q = find_colon(p);
    if (q == NULL) error("expected separator");
...
#if !ENABLE_FEATURE_MAKE_EXTENSIONS
#if ENABLE_FEATURE_MAKE_POSIX_2024
      if (!POSIX_2017 && strcmp(p, ".WAIT") == 0) continue;
#endif
      np = newname(p);
      dp = newdep(np, dp);
#else
        ...
        if (ttype != T_NORMAL) {
          ...
          if (IF_FEATURE_MAKE_EXTENSIONS(posix&&) 1) {
            ...
            if ((ttype & T_INFERENCE)) {
              if (semicolon_cmd) error_in_inference_rule("'; command'");
              seen_inference = TRUE;
            }
            if ((ttype & T_COMMAND) && !cp &&
                !((ttype & T_INFERENCE) && !semicolon_cmd))
              error("commands required for %s", p);
            if (!(ttype & T_COMMAND) && cp) error_not_allowed("commands", p);
          }
...
#undef p
  ...
  if (clevel != old_clevel) error("invalid conditional");
#endif
}
