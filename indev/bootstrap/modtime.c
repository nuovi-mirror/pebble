 ...
 */
static size_t argetnum(const char* str, int len) {
  const char* s;
  size_t val = 0;

  for (s = str; s < str + len && isdigit(*s); s++) {
    // Half-hearted attempt to avoid overflow
    if (val > (INT_MAX - 1) / 10) break;
    val = val * 10 + *s - '0';
  }
  if (s != str + len && *s != ' ') error("invalid archive");
  return val;
}
 ...
 */
static time_t arsearch(FILE* fd, const char* member) {
  struct ar_hdr hdr;
  char *s, *t, *names = NULL;
  ...
  top:
    ...
    if (hdr.ar_name[0] == '/') {
      if (hdr.ar_name[1] == ' ') {
        ...
        names = xmalloc(len);
        if (fread(names, 1, len, fd) != len) error("invalid archive");
        ...
        if (offset > max_offset) error("invalid archive");
        t = names + offset;
      } else {
        error("invalid archive");
      }
    ...
    s = strchr(t, '/');
    if (s == NULL) error("invalid archive");
    *s = '\0';
...
static time_t artime(const char* archive, const char* member) {
  FILE* fd;
  char magic[SARMAG];
  size_t len;
  time_t mtime;
...
void modtime(struct name* np) {
  ...
  if (member) {
    ...
    if (errno != ENOENT) error("can't open %s: %s", name, strerror(errno));
    np->n_tim.tv_sec = 0;
    np->n_tim.tv_nsec = 0;
  } else {
