...
static void usage(int exit_code) {
  ...
  fprintf(
      ...
      "Usage: %s" IF_FEATURE_MAKE_EXTENSIONS(
          " [--posix] [-C path]") " [-f makefile]" IF_FEATURE_MAKE_POSIX_2024(" [-j num]")
 ...
 * fargv.
 */
static char** expand_makeflags(int* fargc) {
  const char *m, *makeflags = getenv("MAKEFLAGS");
  char *p, *argstr;
  int argc;
  char** argv;

  if (makeflags == NULL) return NULL;

  while (isblank(*makeflags)) makeflags++;

  if (*makeflags == '\0') return NULL;
 ...
 */
static char** process_macros(char** argv, int level) {
  char* equal;
  ...
  for (; *argv; argv++) {
    ...
    if (equal - 1 > *argv && equal[-1] == ':') {
      ...
      if (equal - 2 > *argv && equal[-2] == ':') {
        if (POSIX_2017) error("invalid macro assignment");
      ...
      {
#if ENABLE_FEATURE_MAKE_EXTENSIONS
        if (posix) error("invalid macro assignment");
        colon = equal - 1;
        immediate = M_IMMEDIATE;
        *colon = '\0';
#else
 ...
 */
static void update_makeflags(void) {
  int i;
  char optbuf[] = "-?";
  char* makeflags = NULL;
  char *macro, *s;
...
static void make_handler(int sig) {
  signal(sig, SIG_DFL);
  remove_target();
  kill(getpid(), sig);
}

static void init_signal(int sig) {
  ...
  if (sa.sa_handler != SIG_IGN) sigaction(sig, &new_action, NULL);
}
 ...
 */
static void mark_special(const char* special, uint32_t oflag, uint16_t nflag) {
  struct name* np;
  struct rule* rp;
  struct depend* dp;
  int marked = FALSE;
...
int main(int argc, char** argv) {
    ...
    do {
      ...
      if (getcwd(cwd, len)) {
        if (!useenv) {
          ...
          if (envcwd && strcmp(cwd, envcwd) != 0) setenv("CURDIR", cwd, 1);
        }
  ...
  if (!found_target) {
    if (!firstname) error("no targets defined");
