 ...
 */
static void vwarning(FILE* stream, const char* msg, va_list list) {
  const char* m = NULL;
  int d = 0;
...
void error_unexpected(const char* s) { error("unexpected %s", s); }
#endif
...
unsigned int getbucket(const char* name) {
  ...
  while (*p) hashval ^= (hashval << 5) + (hashval >> 2) + *p++;
