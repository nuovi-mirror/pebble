...
struct macro* getmp(const char* name) {
  ...
  for (mp = macrohead[getbucket(name)]; mp; mp = mp->m_next)
    if (strcmp(name, mp->m_name) == 0) return mp;
  return NULL;
}

static int is_valid_macro(const char* name) {
  const char* s;
  for (s = name; *s; ++s) {
    ...
    if (*s == '=') return FALSE;
#if ENABLE_FEATURE_MAKE_POSIX_2024
    if (isblank(*s) || iscntrl(*s)) return FALSE;
#endif
  }
  return TRUE;
}
...
#if ENABLE_FEATURE_MAKE_EXTENSIONS
static int potentially_valid_macro(const char* name) {
  int ret = FALSE;
