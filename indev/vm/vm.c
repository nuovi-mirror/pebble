...
Instruction makeIR(char* line, Arena* tempAlloc, Arena* persistAlloc,
  ...
  if (instrFromMap != NULL) return *instrFromMap;
...
/* XXX audit this */
Value resolve_literal(char* buff, unsigned long buffsize, Value* val,
                      VarMap* vars, Arena* tempAlloc, Arena* persistAlloc) {
...
Value resolve_forced_eval(char* buff, unsigned long buffsize, Value* val,
                          VarMap* vars, Arena* tempAlloc, Arena* persistAlloc) {
  valuetostr(buff, buffsize, *val);
...
unsigned long resolvefunction(unsigned long pc, FuncMap* funcs, Stack* stack,
                              Instruction* program, Arena* scratchAlloc,
                              const char* funcname,
  ...
  int tpos =
      (pc + 1 < instruction_count) && program[pc + 1].Opcode == Opcode_End;
...
unsigned long interpret(Instruction* instr, Instruction* program,
                        unsigned long long seed,
                        unsigned long instruction_count, VarMap* vars,
                        FFIvars* ffivars, Stack* stack, FuncMap* funcs,
                        unsigned long pc, Arena* persistAlloc,
  ...
  switch (instr->Opcode) {
    case Opcode_New: {
      ...
      switch (instr->FirstOperand.Addressing) {
        ...
        default:
          print(
              "ERROR: VM: INTERPRETER: NEW: UNKNOWN ADDRESSING MODE ON OPERAND "
              "ONE\n");
          exitproc(1);
          break;
      }
      ...
      if (existing != NULL)
        ...
        *valptr = val;
        ...
        char* name =
            alloc(persistAlloc, namelen + 1); /* +1 for the null terminator */
    ...
    case Opcode_If: {
      ...
      switch (instr->SecondOperand.Addressing) {
        ...
        /* in this case, a pointer refers to a pointer to an expression, which
         * is compatable with the forced_eavl addressing mode, so we can just
         * copy that */
        case addrmode_pointer: {
          char* buff = alloc(tempAlloc, limits_functions_namesize);
          result = resolve_forced_eval(buff, limits_functions_namesize,
    ...
    case Opcode_Call: {
      ...
      return resolvefunction(pc, funcs, stack, program, scratchAlloc, funcname,
                             instruction_count);
      break;
    }
    ...
    case Opcode_Escape: {
      ...
      switch (instr->FirstOperand.Addressing) {
        case addrmode_bare:
          valuetostr(name, limits_escapes_namesize, instr->FirstOperand.Data);
          break;

        default:
          print(
              "ERROR: INTERPRETER: ESCAPE: ADDRESSING MODE IS UNSUPPORTED!\n");
          exitproc(1);
      }

      callEscape(name, seed, ffivars, scratchAlloc, tempAlloc, persistAlloc);
      break;
    }
    ...
    case Opcode_Internal_PRINT: {
      ...
      if (getstrlen(buf) >= 32) {
        print(
            "\nERROR: INTERPRETER: _PRINT (INTERNAL INSTRUCTION): DATA TOO "
            "LONG!\n");
        exitproc(1);
      }
    ...
    case Opcode_Internal_PRINT2: {
      ...
      switch (data.Type) {
        case type_word:
          print("WORD,  DATA: ");
          valuetostr(buf, limits_instructions_varnamesize, data);
          break;
        case type_sword:
          print("SWORD, DATA: ");
          valuetostr(buf, limits_instructions_varnamesize, data);
...
int vmmain(Args cliargs, Stack* stack, unsigned long long seed) {
  ...
  struct Arena* persistAlloc = initAlloc(mem_arena_persistAlloc_backing,
                                         limits_allocator_persist_maxmem);
  struct Arena* scratchAlloc = initAlloc(mem_arena_scratchAlloc_backing,
  ...
  */
  ...
  char* file = alloc(
      tempAlloc, filesize + 1); /* +1 for null terminator readfile() appends */
  ...
  unsigned long current_instruction_buffersize =
      limits_instructions_initbuffersize;
  ...
  while (pc < current_instruction_count) {
    pc = interpret(&program[pc], program, seed, current_instruction_count,
                   &vars, ffivars, stack, &funcs, pc, persistAlloc,
                   scratchAlloc, tempAlloc);
